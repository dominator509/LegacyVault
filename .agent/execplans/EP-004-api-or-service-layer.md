...
- Portable export encryption/signing requires the `packages/crypto` boundary assigned to EP-006, and production identity resolution requires the `packages/auth` boundary assigned to EP-006. EP-004 cannot honestly complete those integrations before the independent EP-006 continuation slice exists.
