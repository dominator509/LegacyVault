...
- RUN_COMPLETE is not reached because the authenticated Stripe billing round trip, Resend/Turnstile/R2/Redis/database/GHCR/Fly production values and staging proofs are absent, and the required safe evidence-reference files have not been placed under `compliance/evidence/`
...
| Billing | Engineering complete; Stripe account and Price authenticated | Entitlements, checkout/portal adapters, signatures, event ordering, cancellation/refund state, replay protection, authenticated test-account balance and exact active recurring Price probes | Customer portal, delivered webhook and checkout/cancellation/refund live-fire |
...
| EXT-005 | Stripe | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ESSENTIAL` | Test account and exact active recurring Price probes pass; configure the portal/webhook endpoint, then deliver checkout, subscription, cancellation and refund events | `sh scripts/probes/stripe.sh`; then preflight and live-fire |
...
3. Configure the Stripe test portal/webhook endpoint and complete checkout/portal/webhook/refund/cancellation live-fire. The account and exact active recurring Price probes already pass.
