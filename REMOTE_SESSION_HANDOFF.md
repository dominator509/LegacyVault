...
| Authentication/authorization | Engineering complete locally; Turnstile test keys verified | Better Auth sessions, passkeys, TOTP, recovery, reset, revocation, explicit authenticated-user SQL ownership plus forced RLS, policy and Chrome proofs; real Siteverify deterministic pass/fail testing | Real Turnstile widget, frontend/server enforcement, controlled hostname/browser-token proof and physical-authenticator cross-browser proof; operator asserts external security approval |
...
Latest full local counts: 68 unit tests, 76 integration tests, 7 browser E2E tests, 39/39 contract operations with 1,727/1,727 generated cases, 21 Next.js routes, security static assertions, zero known dependency vulnerabilities, smoke, prior real DeepSeek live-fire, authenticated Stripe Checkout/portal live-fire, authenticated Resend send/retrieve live-fire, Cloudflare Turnstile public-pair Siteverify live-fire, authenticated k6, local migration idempotence, checksummed backup/restore comparison, and combined OCI runtime rehearsal all pass. A completed Standard security scan found one low-severity membership-query defense-in-depth gap; it is fixed and protected by SQL, integration, and static assertions. Schemathesis still reports visible unauthenticated/missing-fixture/schema warnings; authenticated success paths are covered separately, but authenticated stateful fuzzing remains deferred rather than hidden.
...
| EP-006 | Engineering complete but externally unverified | Auth/security/compliance mechanisms and public-pair Turnstile Siteverify pass locally; real widget/enforcement and external evidence remain |
...
| EXT-007 | Turnstile | `TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY` plus scoped Cloudflare authorization | Public test pair passes; create one controlled-hostname widget/siteverify Worker and wire the frontend/server gate | `sh scripts/turnstile-live-fire.sh`; then real-widget auth E2E and preflight |
...
- Stripe delivered webhooks/payment lifecycle, real Turnstile widget/enforcement, production Resend domain/events, hosted PostgreSQL/Redis/R2, GHCR, Fly, DNS/WAF and managed restore behavior have not been authenticated from this workspace, even where provisioning is operator-asserted.
...
4. Supply a scoped Cloudflare token and production hostname for Turnstile Spin; rotate the supplied Resend key, verify the production sender domain, and complete auth abuse-prevention plus email delivery-event proofs.
