...
- RUN_COMPLETE is not reached because the approved Stripe Price ID and authenticated billing round trip, Resend/Turnstile/R2/Redis/database/GHCR/Fly production values and staging proofs are absent, and the required safe evidence-reference files have not been placed under `compliance/evidence/`

The DeepSeek credential and authenticated inference are verified locally. The credential remains only in ignored `.env`; it is not committed or repeated here and should be rotated because it was pasted into chat. The Stripe test secret passed the authenticated balance probe on 2026-08-08, and the supplied webhook secret is stored only in ignored `.env`; neither value is committed or repeated here. The supplied publishable key is not a blueprint-declared or consumed variable and was therefore not added. The operator states that counsel, all policies and retention policies, Stripe pricing, insurance, incident contacts, regions, data-broker status, external security review, hosting, and database provisions are complete. Those statements are recorded as operator assertions, not fabricated evidence; safe reference records and authenticated probes are still required by the immutable ship gate.
...
| Billing | Engineering complete; Stripe account authentication verified | Entitlements, checkout/portal adapters, signatures, event ordering, cancellation/refund state, replay protection, authenticated test-key balance probe | Approved Price ID, customer portal, delivered webhook and checkout/cancellation/refund live-fire |
...
| EXT-005 | Stripe | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ESSENTIAL` | Test key probe passed and webhook secret is stored locally; supply the approved Price ID and configure the portal/webhook endpoint, then deliver checkout, subscription, cancellation and refund events | `sh scripts/probes/stripe.sh`; then preflight and live-fire |
...
- Stripe, Turnstile, Resend, hosted PostgreSQL/Redis/R2, GHCR, Fly, DNS/WAF and managed restore behavior have not been authenticated from this workspace, even where provisioning is operator-asserted.
...
3. Supply `STRIPE_PRICE_ESSENTIAL`, configure the Stripe test portal/webhook endpoint, and complete checkout/portal/webhook/refund/cancellation live-fire. The secret-key account probe already passes.
